    ...
    article.innerHTML = `
        ...
        <div class="post-content">
            ...
            <div class="post-action">
                <button type="button" class="post-action-button" data-like>♡ いいね</button>
                ${state.isOwn ? '<button type="button" class="post-action-button" data-save>🔖 保存</button>' : ''}
            </div>
        </div>`;
    applyAvatar(article.querySelector('.post-icon'), profile || state.viewedProfile);
    article.querySelector('[data-like]')?.addEventListener('click', async () => {
        try { await setPostLiked(state.authUser.id, post.id, true); toast('いいねしました。'); } catch (e) { alert(e.message); }
    });
    article.querySelector('[data-save]')?.addEventListener('click', async () => {
        try { await setPostSaved(state.authUser.id, post.id, true); toast('保存しました。'); } catch (e) { alert(e.message); }
    });
    if (editable) {
