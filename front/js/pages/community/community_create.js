...
async function createCommunity() {
    ...
    try {
        ...
        const categoryName = communityCategorySelect.value;
        ...
        if (!category) {
            throw new Error(`カテゴリー「${categoryName}」がデータベースにありません。先にカテゴリ初期データSQLを実行してください。`);
        }
        ...
        showToast("コミュニティを作成しました。", "success");

        window.setTimeout(() => {
            window.location.href =
                `community_home.html?id=${encodeURIComponent(community.id)}`;
        }, 700);
        ...
        showToast(
            error?.message || "コミュニティを作成できませんでした。",
            "error"
        );
    }
}
